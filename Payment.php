...
session_start();
	...
	if (isset($_POST["btn"]))
	{
		...
		$sql = "INSERT INTO paymentdetails (Name, Cardnumber, Expirydate, Cvv, Amount, Apartment_ID ) VALUES ('$name', '$card_number', '$expiry_date', '$cvv', $amount, '$apartmentID' )";
...
<body style="background-color:#68D2E8;">
  ...
  <h1>Payment Form</h1>
	...
	<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="post" onsubmit="validateForm()">
		...
		<input type="hidden" name="apartmentID" value="<?php echo $_GET['apartment_Id']; ?>">

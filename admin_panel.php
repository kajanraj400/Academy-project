...
<?php
	...
	if( isset( $_SESSION['user_email']))
	{
		...
		if ($results3->num_rows > 0) 
		{
			...
			echo '<table border="1">'.
				 '<tr><th>Apartment ID</th><th>Buyer ID</th><th>Name</th><th>Cardnumber</th><th>Expirydate</th>'.
			     '<th>Cvv</th><th>Amount</th><th>Delete</th></tr>';
			...
			while ($row3 = $results3->fetch_assoc())
			{
				...
				$Cvv2 = $row3["Cvv"];
				$Amount2 = $row3["Amount"];
				$Apartment_ID = $row3["Apartment_ID"];

			   echo '<tr><td>'.$Apartment_ID.'</td><td>'.$ID2.'</td><td>'.$Name2.'</td><td>'.$Cardnumber2.'</td><td>'.$Expirydate2.'</td>'.
					'<td>'.$Cvv2.'</td><td>'.$Amount2.'</td>'.
					'<td><a href="approved.php?apartment_ID='.$Apartment_ID.'&Cardnumber='.$Cardnumber2.'">'.
					'<button>Delete</button></a></td></tr>';
			}
	...
	if( isset( $_SESSION['user_email']))
	{
		$sql4 = "SELECT * FROM SOLD";
		$results4 = $conn->query($sql4);
	   
		if ($results4->num_rows > 0) 
		{
			echo"<h1>Apartment Sold Details</h1>";
			echo '<table border="1">'.
				 '<tr><th>Apartment ID</th><th>Apartment Name</th><th>Owner Name</th><th>Owner Number</th>'.
			     '<th>Address</th><th>Price</th><th>Card Number</th></tr>';
		
			while ($row4 = $results4->fetch_assoc())
			{
				$Apartment_ID4 = $row4["Apartment_ID"];
				$Name4 = $row4["Name"];
				$OwnerName4 = $row4["OwnerName"];
				$OwnerNumber4 = $row4["OwnerNumber"];
				$Address4 = $row4["Address"];
				$Price4 = $row4["Price"];
				$CardNumber4 = $row4["CardNumber"];

			   echo '<tr><td>'.$Apartment_ID4.'</td><td>'.$Name4.'</td><td>'.$OwnerName4.'</td><td>'.$OwnerNumber4.'</td><td>'.$Address4.'</td>'.
					'<td>'.$Price4.'</td><td>'.$CardNumber4.'</td></tr>';
			}
		...
		{
			echo "<h1>No Sold Details Available</h1>";
		}

...
function DeleteUser() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/deletuserdeatiles")
      .then((result) => {
        setUser(result.data);
  ...
  function logout() {
    const logoutconform = window.confirm(
  ...
  function handleConfirmAdd() {
    axios
      .post("http://localhost:5000/adduser", { email: selectedEmail })
      .then((result) => {
  ...
  return (
    <div className="w-11/12 mx-auto">
      ...
      <h2 className="inline-block text-blue-600 bg-white bg-w-cover font-bold text-xl ml-5 py-3 px-5">
        Total Customers Removed: {searchResults.length}
      </h2>
        ...
        <table className="w-full border-collapse border border-gray-300 bg-white rounded-xl relative z-10">
          ...
          <tbody>
            {searchResults.map((ob) => (
              <tr
                key={ob._id}
                style={{ backgroundColor: ob % 2 === 0 ? "#fafafa" : "#fff" }}
              >
        ...
        <div className="modal-overlay">
          <div className="modal">
            ...
            <div className="modal-buttons">
              ...
              <button className="cancel-btn" onClick={handleCanceAdd}>
                Cancel
              </button>
            </div>
          </div>
